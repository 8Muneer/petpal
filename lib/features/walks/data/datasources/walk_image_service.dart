...
class WalkImageService {
  ...
  Future<String> uploadPetImage(String requestId, XFile file) async {
    final ref = _storage.ref().child('walk_pet_images/$requestId');
    await ref.putFile(
      File(file.path),
  ...
  Future<List<String>> uploadPetImages(
    ...
    for (int i = 0; i < files.length; i++) {
      ...
      final ref =
          _storage.ref().child('walk_pet_images/${requestId}_img$i');
      await ref.putFile(
        File(files[i].path),
        SettableMetadata(contentType: 'image/jpeg'),
      );
  ...
  Future<XFile?> pickMultipleImages() async {
