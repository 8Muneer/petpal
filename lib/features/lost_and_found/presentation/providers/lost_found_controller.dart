...
class LostFoundState {
  final int selectedTabIndex; // 0 for Lost, 1 for Found
  final String searchQuery;
  final LostFoundViewType viewType;
  final String? selectedPetType; // null for all, 'dog', 'cat', etc.
...
class LostFoundController extends StateNotifier<LostFoundState> {
  ...
  void setPetType(String? petType) => state = state.copyWith(selectedPetType: petType);
...
/// A unified provider that filters posts based on the current controller state.
final filteredLostFoundPostsProvider = Provider<AsyncValue<List<LostFoundPost>>>((ref) {
  final state = ref.watch(lostFoundControllerProvider);
  
  // Watch the appropriate raw stream based on tab
  ...
  return postsAsync.whenData((posts) {
    ...
    // 1. Filter by search query (name, breed, area, description)
    ...
    // 2. Filter by pet type (species)
    if (state.selectedPetType != null) {
      filtered = filtered.where((post) => post.species == state.selectedPetType).toList();
    }
